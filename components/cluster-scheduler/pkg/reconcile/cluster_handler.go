...
func NewClusterEventHandler(cs service.ClusterService, sp sentryrpc.SentryPool, cp configrpc.ConfigPool) ClusterEventHandler {
...
func (h *clusterEventHandler) handleClusterEvent(ev event.Resource) {
	...
	reconciler := NewClusterReconciler(h.cs, h.sp, h.cp)
