...
type clusterReconciler struct {
	...
	sp sentryrpc.SentryPool
	cp configrpc.ConfigPool
}

// NewClusterReconciler returns new cluster reconciler
func NewClusterReconciler(cs service.ClusterService, sp sentryrpc.SentryPool, cp configrpc.ConfigPool) ClusterReconciler {
	return &clusterReconciler{cs: cs, sp: sp, cp: cp}
}
...
func (r *clusterReconciler) handleClusterBootstrapAgent(ctx context.Context, cluster *infrav3.Cluster) error {
	...
	try := func() error {
		err := bootstrapagent.CreateForCluster(ctx, r.sp, r.cp, cluster)
		return err
	}
	...
	onSuccess := func() error {
		...
		if err == nil {
			fmt.Print("annotations in reconciler before update :: ", cluster.Metadata.Annotations)
			fmt.Print("id in reconciler before update :: ", cluster.Metadata.Id)
