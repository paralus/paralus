...
func testGetKubectlCommands(tag string) func(t *testing.T) {
	return func(t *testing.T) {
		...
		mock.ExpectQuery(regexp.QuoteMeta(`SELECT "auditlog"."tag", "auditlog"."time", "auditlog"."data" FROM "audit_logs" AS "auditlog" WHERE ((tag = '` + tag + `') OR (tag = 'kubectl_api' AND data->>'session_type' != 'browser shell')) AND (data->>'project' = '` + project + `') AND (time between now() - interval '` + timefrom + `' and now()) ORDER BY "time" desc`)).
			WillReturnRows(sqlmock.NewRows([]string{"tag", "time", "data"}).AddRow(tag, time.Now(), auditrecord).AddRow(tag, time.Now(), auditrecordtwo))

		mock.ExpectQuery(regexp.QuoteMeta(`SELECT count(1) as count, data->>'project' as key FROM "audit_logs" WHERE ((tag = '` + tag + `') OR (tag = 'kubectl_api' AND data->>'session_type' != 'browser shell')) AND (data->>'project' = '` + project + `') AND (time between now() - interval '` + timefrom + `' and now()) GROUP BY data->>'project'`)).
			WillReturnRows(sqlmock.NewRows([]string{"count", "key"}).AddRow(1, "project"))

		mock.ExpectQuery(regexp.QuoteMeta(`SELECT count(1) as count, COALESCE(data->'actor'->'account'->>'username', data->>'username') as key FROM "audit_logs" WHERE ((tag = '` + tag + `') OR (tag = 'kubectl_api' AND data->>'session_type' != 'browser shell')) AND (data->>'project' = '` + project + `') AND (time between now() - interval '` + timefrom + `' and now()) GROUP BY COALESCE(data->'actor'->'account'->>'username', data->>'username')`)).
			WillReturnRows(sqlmock.NewRows([]string{"count", "key"}).AddRow(1, "username"))

		mock.ExpectQuery(regexp.QuoteMeta(`SELECT count(1) as count, data->>'type' as key FROM "audit_logs" WHERE ((tag = '` + tag + `') OR (tag = 'kubectl_api' AND data->>'session_type' != 'browser shell')) AND (data->>'project' = '` + project + `') AND (time between now() - interval '` + timefrom + `' and now()) GROUP BY data->>'type'`)).
			WillReturnRows(sqlmock.NewRows([]string{"count", "key"}).AddRow(1, "type"))
